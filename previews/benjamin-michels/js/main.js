...
(function () {
  ...
  var rDeal = document.getElementById('rDeal');
  if (rLeads && rDeal) {
    var LOST = 0.35, CLOSE = 0.25;
    var fmt = function (n) { return n.toLocaleString('de-DE'); };
    function recalc() {
      var leads = parseInt(rLeads.value, 10);
      var deal = parseInt(rDeal.value, 10);
      document.getElementById('vLeads').textContent = leads;
      document.getElementById('vDeal').textContent = fmt(deal) + ' €';
      var annual = Math.round(leads * LOST * CLOSE * deal * 12);
      document.getElementById('calcOut').textContent = fmt(annual) + ' €';
    }
    rLeads.addEventListener('input', recalc);
    rDeal.addEventListener('input', recalc);
    recalc();
  }
